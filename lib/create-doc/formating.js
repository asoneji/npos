...
Formating.prototype.getBarcodeposition = function() {
  return this.barcodeposition;
};

Formating.prototype.setBarcodeposition = function(setBarcodepositionValue) {
...
Formating.prototype.setBarcodetype = function(setBarcodetypeValue) {
  ...
  switch (setBarcodetypeValue) {
    ...
    case 68:
      ...
      break;
    case 70:
      this.barcodetype = "ITF";
      break;
    case 71:
      this.barcodetype = "CODABAR";
      break;
    case 72:
...
Formating.prototype.addBarcode = function() {
  ...
  //var barcodeTemp = '<svg class=\"barcode\" jsbarcode-format=\"upc\" jsbarcode-value=\"123456789012\" jsbarcode-textmargin=\"0\" jsbarcode-fontoptions=\"bold\"> </svg>';
  var barcodeTemp = '<svg class=\"barcode\" jsbarcode-format=' +  this.getBarcodetype() + ' jsbarcode-value=' + this.getBarcodetext() + ' jsbarcode-textposition=' + this.getBarcodeposition()  + ' > </svg>';
  $(barcodeTemp).appendTo($( ".receipt" ));
  //$(barcodeTemp).addClass(formatingList).appendTo($( ".receipt" ));
  //$('<div>'+ barcodeValue + '</div>').addClass(formatingList).appendTo($( ".receipt" ));
...
/*
  //$('<div>'+ JsBarcode + '</div>').addClass("barcode").appendTo($( ".receipt" ));
  //var canvas = new Canvas();
  //JsBarcode(canvas, "Hello");
  ...
  //$('<div>'+ JsBarcode + '</div>').addClass(formatingList).appendTo($( ".receipt" ));
*/
...
Formating.prototype.addText = function(textValue) {
  ...
  if(this.docType === this.DOC_TYPE_PDF) {
    ...
    //TODO - need to add underline, double height, double width, doublestrike, justification, color, leftmargin formatting to pdf
