...
Formating.prototype.resetDefaultSettings = function() {
  ...
  this.barcodetype;
  this.barcodetext;
  //Adding all the text with span inside of a div. This allows us to parse multiple escpos command on one line and render correctly
  this.htmltags = [];
  this.parentFormating = '';
  this.textSizeWidth = 1;
  this.textSizeHeight = 1;
};
...
Formating.prototype.getFormatingList = function() {
  ...
  if(this.getTextSizeWidth() > 1 || this.getTextSizeHeight() > 1)
  {
    //textSize = 'display:inline-block; transform:';
    formatingList += "textsize "
  }
  ...
  //need to add justification to parent div tag
  if(this.parentFormating == '')
  {
    this.parentFormating = "justification_" + this.justification.toLowerCase() + " ";
  }
...
Formating.prototype.setTextSize = function(setTextSizeWidthValue, setTextSizeHeightValue) {

  if(setTextSizeWidthValue > 0 && setTextSizeWidthValue < 9)
  {
    this.textSizeWidth = setTextSizeWidthValue;
  }

  if(setTextSizeHeightValue > 0 && setTextSizeHeightValue < 9)
  {
    this.textSizeHeight = setTextSizeHeightValue;
  }

}

Formating.prototype.getTextSizeWidth = function() {
  return this.textSizeWidth;
};

Formating.prototype.getTextSizeHeight = function() {
...
Formating.prototype.addBarcode = function() {
  ...
  if(this.getBarcodetextposition() == this.BARCOD_POSITION_0_TYPE)
  {
    var barcodeTemp = '<svg class=\"barcode\" jsbarcode-format=' +  this.getBarcodetype() + ' jsbarcode-value=' + this.getBarcodetext() + ' jsbarcode-textposition=' + this.getBarcodetextposition()
      + ' jsbarcode-width=' + this.getBarcodewidth()  + ' jsbarcode-height=' + this.getBarcodeheight() + ' jsbarcode-displayvalue=false jsbarcode-flat=true > </svg>';
  }
  else{
    var barcodeTemp = '<svg class=\"barcode\" jsbarcode-format=' +  this.getBarcodetype() + ' jsbarcode-value=' + this.getBarcodetext() + ' jsbarcode-textposition=' + this.getBarcodetextposition()
      + ' jsbarcode-width=' + this.getBarcodewidth()  + ' jsbarcode-height=' + this.getBarcodeheight()  + ' jsbarcode-flat= true > </svg>';
  }
  this.htmltags.push($('<span>' + barcodeTemp + '</span><br>').addClass(formatingList));
};
...
Formating.prototype.addImage = function(imageValue) {
  ...
  this.htmltags.push($('<span><img src='+'></img></span><br>').attr("id", imageID).addClass(formatingList))
...
Formating.prototype.addText = function(textValue) {
  ...
  if(this.docType === this.DOC_TYPE_PDF) {
    ...
    var textValue= textValue.split("\n");

    var containsNewLine = textValue.length > 1 ? true : false;
    textValue =  textValue.join("<br />");

    var leftMarginStyle = '';
    if(this.getLeftmargin())
    {
      //This calculation works for my printer EPSON TM-T20II maybe different for other printers
      var percent = this.getLeftmargin() * (21/100);
      //TODO - add the style to css class using cheerio
      leftMarginStyle = 'style="margin-left: '+ percent + 'mm;"';
    }

    //TODO - need to fix the padding and margin
    var textSizeWidth = '';
    var textSizeHeight = '';
    var textSize = '';
    var textSizePadding = '';
    ...
    if(this.getTextSizeWidth() > 1 || this.getTextSizeHeight() > 1)
    {
      console.log("Width: " + this.getTextSizeWidth() + " height: " +  this.getTextSizeHeight());
      //textSize = 'display:inline-block; transform:';
      textSize = 'transform:';
    }

    if(this.getTextSizeHeight() > 1)
    {
      textSizeHeight = ' scaleY('+ (.5 + (.5 * this.getTextSizeHeight())) +')';
      //textSizeHeight = ' scaleY('+ this.getTextSizeHeight() +')';
      //textSizePadding += ' padding-top: ' + (this.getTextSizeHeight()+1)  + 'mm; padding-bottom: ' + (this.getTextSizeHeight()+1)  + 'mm;';
      //textSizePadding += ' margin-top: ' + (this.getTextSizeHeight()+1)  + 'mm; margin-bottom: ' + (this.getTextSizeHeight()+1)  + 'mm;';
    }

    if(this.getTextSizeWidth() > 1)
    {
      textSizeWidth = ' scaleX('+ (.5 + (.5 * this.getTextSizeWidth())) +') translate(-1px,0px); transform-origin:left;';
      //textSizeWidth = ' scaleX('+ this.getTextSizeWidth() +') translate(-1px,0px); transform-origin:left;';
      //textSizePadding += ' padding-left: ' + (this.getTextSizeWidth()+1)  + 'mm; padding-right: ' + (this.getTextSizeWidth()+1)  + 'mm;';
      //textSizePadding += ' margin-left: ' + (this.getTextSizeWidth()+1)  + 'mm; margin-right: ' + (this.getTextSizeWidth()+1)  + 'mm;';
    }

    if(textSize !== '')
    {
      //console.log("style="+textSize+textSizeWidth+textSizeHeight+textSizePadding);
      this.htmltags.push($('<span>'+ textValue + '</span>').attr("style", textSize + textSizeHeight + ' ' + textSizeWidth + textSizePadding).addClass(formatingList));
    }
    else{
      this.htmltags.push($('<span>'+ textValue + '</span>').addClass(formatingList));
    }
    ...
    if(containsNewLine) {
      ...
      var divContent = '<div '+ leftMarginStyle + '>';
      for (i = 0; i < this.htmltags.length; i++) {
        divContent += this.htmltags[i];
      }
      divContent += '</div>';

      $(divContent).addClass(this.parentFormating).appendTo(".receipt");
...
Formating.prototype.finish = function() {
  ...
  else
  {
    ...
    //TODO - Check if the __dirname is ok or should the path be different
    //TODO - confirm we don't need the bellow
    /*
        for (i = 0; i < this.htmltags.length; i++) {
          $('<div>'+this.htmltags[i]+'</div>').appendTo(".receipt");
        }
    */
